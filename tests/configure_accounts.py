...
"""
This script walks the user through creating and configuring two accounts in
...
class User:
    ...
    def _oneshot_login(self):
        output = self._run_command(
            "oneshot", "--paperkey", self.paperkey, "--username", self.username
        )
    ...
    def invite_to_team(self, team, username):
        ...
        :return:            None
        """
        assert b"Success!" in self._run_command(
            "team", "add-member", team, "--user=%s" % username, "--role=writer"
        )

    def create_team_and_invite(self, team, username):
        """
        Create a team and invite the given user to it as an admin. Team can be
        a subteam but the parent teams must exist
        :param team:    A team name. Eg "foo", "foo.bar"
    ...
    def create_channel(self, team, channel):
        ...
        :param team:        The name of the team that the channel will be in
        :param channel:     The name of the channel (without any '#')
        :return:            None
        """
        assert b"Success!" in self._run_command(
            "chat", "create-channel", team, channel, "--topic-type", "chat"
        )
    ...
    def join_channel(self, team, channel):
        ...
        :return:            None
        """
        assert b"" == self._run_command(
            "chat", "join-channel", team, channel, "--topic-type", "chat"
        )
...
def make_user(purpose):
    """
    Prompts the user via stdout and stderr in order to either create a new user
    ...
    if answer == "y":
        ...
        print(
            "TEST SETUP - Follow the prompts in order to create a new \
            account..."
        )
        ...
        os.system(
            "keybase --home %s signup --no-email --username %s"
            % (home_dir, example_username)
        )
