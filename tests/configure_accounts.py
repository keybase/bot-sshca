...
class User:
    ...
    def create_team_and_invite(self, team, username, *usernames):
        """
...
def write_env_files(kssh_user, ca_user, mofn_approver, parent_team):
...
if __name__ == '__main__':
    ...
    mofn_approver = make_user("the M of N request approver")

    parent_team = secure_random_str(16)
    ca_user.create_team_and_invite(parent_team, kssh_user.username, mofn_approver.username)

    ca_user.create_team_and_invite(parent_team + ".ssh", kssh_user.username, mofn_approver.username)
    ca_user.create_channel(parent_team + ".ssh", "ssh-provision")
    kssh_user.join_channel(parent_team + ".ssh", "ssh-provision")
    mofn_approver.join_channel(parent_team + ".ssh", "ssh-provision")
    ca_user.create_team_and_invite(parent_team + ".ssh.staging", kssh_user.username, mofn_approver.username)
    ca_user.create_team_and_invite(parent_team + ".ssh.prod", mofn_approver.username)
    ca_user.create_team_and_invite(parent_team + ".ssh.root_everywhere", kssh_user.username, mofn_approver.username)

    ca_user.create_team_and_invite(parent_team + ".secondary", kssh_user.username, mofn_approver.username)

    write_env_files(kssh_user, ca_user, mofn_approver, parent_team)
